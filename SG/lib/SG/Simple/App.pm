...
sub _update_cron {
    ...
    unless( $sg ) {
        ...
        $sg = new Yote::RootObj( {
            name    => 'SG',
            enabled => 1,
            script  => 'use Yote::WebRoot; use SG::App; my $r = Yote::
WebRoot::fetch_webroot(); my $a = $r->fetch_app_by_class( "SG::App" ); if( $a ) { print STDERR " SG :: checking turns\n"; $a->_check_turns; print STDERR " SG :: checking turns done\n";  } else { print STDERR "Could not find SG::App\n" };',
...
sub create_game {
    my( $self, $data, $acct, $env ) = @_;
    my $game = new SG::Simple::Game( $data );
